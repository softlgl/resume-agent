  ...
  properties: {
    ...
    projects: {
      ...
      items: {
        ...
        required: ["name", "role", "start", "end", "link", "description"],
        additionalProperties: false,
      },
    },
    skills: {
...
- projects: 项目经历数组，每项 { name(项目名), role(你的角色), start, end, link(项目链接,没有填空), description }
...
export async function structurizeText(text: string, onReasoning?: (delta: string) => void, onContent?: (delta: string) => void): Promise<ResumeContent | null> {
  ...
  // maxTokens 调大：推理模型(reasoning_content)会占用大量 token，默认值易导致 JSON 被截断
  const out = await chatStream(buildPrompt(text), { jsonSchema: EXTRACT_SCHEMA, temperature: 0.1, maxTokens: 6000 }, onReasoning, onContent);
