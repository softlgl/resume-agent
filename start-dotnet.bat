@echo off
chcp 65001 >nul
title 简历助手 (.NET版) - 一键启动
cd /d %~dp0

echo ====================================
echo   简历助手 (Resume Agent) 一键启动 (.NET 版)
echo ====================================
echo.

REM 检查前端依赖是否已安装（node_modules 存在）
if not exist "node_modules" (
  echo [1/3] 首次运行，安装前端依赖中（可能需要几分钟）...
  call npm install
  if errorlevel 1 (
...
echo [2/3] 启动后端服务 (ASP.NET Core :4000) 与前端 (Vite :5173)...
REM 先清理可能残留的旧服务进程，避免旧构建/旧代码继续占用端口
REM 注意：端口与 Node 版一致（4000），两个后端不要同时启动
...
start "resume-server-dotnet" cmd /c "powershell -NoProfile -ExecutionPolicy Bypass -File packages\server-dotnet\dev.ps1 ^> server-dotnet.out.log 2^>^&1"
start "resume-client" cmd /c "npm run dev:client ^> client.out.log 2^>^&1"

echo [3/3] 等待前端服务就绪并打开浏览器...
ping -n 9 127.0.0.1 >nul

REM 等待前端端口就绪（最大 30 秒）
set "READY=0"
...
echo   后端  -> http://localhost:4000  (.NET 10 + ASP.NET Core)
echo.
echo 日志文件：server-dotnet.out.log / client.out.log
echo 关闭窗口不会停止服务，请在对应命令行窗口按 Ctrl+C 结束。
echo 若需停止全部，请运行 stop.bat 或关闭 "resume-server-dotnet" / "resume-client" 窗口。
echo.
pause
